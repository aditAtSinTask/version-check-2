...
const versionCheck2 = async (packageName, version) => {
  ...
  try {
    ...
    if (version !== "latest") {
      latestResponse = await fetch(`${npmRegistryURL}/${packageName}/latest`, {
        headers,
        method: "GET",
      });
    ...
    if (
      ...
      return {
        ...
        data: {
          ...
          latestVersion:
            responseStatus.latestVersion === 404
              ? "no.package.found"
              : responseData.latestVersion.version,
        },
      };
    } else {
